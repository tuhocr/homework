qqPlot_enhanced <- function (x, distribution = "norm", groups, layout, ylim = range(x, 
          ...
          ...) 
{
  ...
  if (!isFALSE(envelope)) {
    envelope <- car:::applyDefaults(envelope, defaults = list(level = 0.95, 
  ...
  id <- car:::applyDefaults(id, defaults = list(method = "y", n = 2, 
  ...
  if (!isFALSE(envelope)) {
    ...
    if (style == "filled") {
      car:::envelope(z, z, lower, upper, col = col.envelope, 
